...
const Salon = () => {
  ...
  // Extract the salon name from the pathname
  const salonName = pathname.split("/")[pathname.split("/").length - 1];
  // Transform the salon name
  const transformed = transformString(salonName);

  return (
    <>
    <Helmet>
                <meta name="description" content={`About Us | ${transformed} Salon`}  />
                <title>{`${transformed} Salon | LUZO`}</title>
